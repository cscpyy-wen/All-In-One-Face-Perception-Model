...
class TransformerDecoderLevel(nn.Module):
    ...
    def get_vis_token_pos_embed(self, shape=None, batch=1):
        if self.cross_pos_embed == 'anchor':
            ...
            mask = torch.zeros(batch, *shape, dtype=torch.bool, device=dev)
            H_n, W_n = shape
    ...
    def forward(self, input):
        ...
        for j in range(self.num_feature_levels):
            ...
            loc_emb = self.get_vis_token_pos_embed(x[self.levels[j]].shape[-2:])
        ...
        # QxNxC
        query_embed = self.query_embed[task_name].weight.unsqueeze(1).repeat(1, bs, 1)
        output = self.query_feat[task_name].weight.unsqueeze(1).repeat(1, bs, 1)
...
class TransformerDecoderLevelAddLevel(nn.Module):
    ...
    def forward(self, input):
        ...
        for j in range(self.new_num_feature_levels):
            ...
            loc_emb = self.get_vis_token_pos_embed(x[self.new_levels[j]].shape[-2:])
